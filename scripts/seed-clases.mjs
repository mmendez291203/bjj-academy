/**
 * Seed del horario real de RUNAJERABJJ en Cosmos DB.
 * Uso: node scripts/seed-clases.mjs
 */
...
console.log("Insertando horario en Cosmos DB...\n");

for (const clase of clases) {
  try {
    await container.items.upsert(clase);
    console.log(`✓ ${clase.dia.padEnd(10)} — ${clase.nombre} (${clase.horaInicio}–${clase.horaFin})`);
  } catch (err) {
    console.error(`✗ ${clase.dia} — ${err.message}`);
  }
}

console.log("\n✅ Horario guardado en Cosmos DB.");
